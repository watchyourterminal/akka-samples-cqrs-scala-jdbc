...
import sample.cqrs.infrastructure.{CborSerializable, EventProcessorSettings}
...
object ShoppingCart {
  ...
  def init(system: ActorSystem[_], eventProcessorSettings: EventProcessorSettings): Unit = {
    // EntityKey는 aggregate root 라고 볼 수 있음
    ClusterSharding(system).init(Entity(EntityKey) { entityContext =>
      ...
      ShoppingCart(system, entityContext.entityId, Set(eventProcessorTag))
    }.withRole("write-model"))
  }

  def apply(system: ActorSystem[_], cartId: String, eventProcessorTags: Set[String]): Behavior[Command] = {
    EventSourcedBehavior
      /* Command 처리 후 무조건 reply를 보내도록 type으로 강제함
       *
       * Command: Persist effect를 주는 Event
       * Event: 검증된 Command를 Event로 전환하여 store
       * State: 현재 상태. 각각의 entity id는 shard에서 고유하며,
       *  replay를 통해 해당 entity의 상태를 복구함
       */
      .withEnforcedReplies[Command, Event, State](
        // Persistent actor 식별자
        // Eventstore에서 사용됨.
        PersistenceId(EntityKey.name, cartId),
        // 가장 기본 State
        State.empty,
        (state, command) => {
          system.log.info(s"Receive command: $command, current state: $state")
          if (state.isCheckedOut) checkedOutShoppingCart(cartId, state, command)
          else openShoppingCart(cartId, state, command)
        },
        // Command persist 성공시 실행
        (state, event) => handleEvent(state, event))
      // persistence query 할때 사용되는 tag
